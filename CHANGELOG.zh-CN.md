...
- **Claude 页面宽度** — 修复 Claude 更新页面布局后，页面宽度设置无法继续加宽对话内容的问题。
- **ChatGPT 新版页面适配** — 修复 ChatGPT 页面改版后的三类问题：会话能被扫描到却无法加入会话管理、净化模式下无法移除「ChatGPT 也可能会犯错」提示、双击公式提示缺少可复制的源码；现已兼容新版侧边栏链接与公式结构。
- **隐藏会话自动恢复** — 修复部分历史会话在会话管理中不可见、即使重新同步也无法恢复的问题；升级后会自动把指向已删除文件夹的会话移回收件箱。
- **适配包提示词插入** — 修复向 LongCat AI 等站点的富文本输入框插入多行提示词时，文本已成功插入却提示"插入失败"的问题。
- **Claude 新版页面适配** — 适配 Claude.ai 新版 DOM 结构，更新侧边栏会话列表、发送按钮、生成检测、禅模式和智能避让的选择器；修复置顶会话再取消置顶后，会话管理中的标题重复显示（变成“标题标题”）的问题。
- **适配包阅读位置恢复** — 修复未支持阅读记录的适配包站点仍可能执行阅读位置恢复的问题，该问题会打断 LongCat AI 等站点点击大纲标题的页内跳转。
- **LongCat AI 大纲跳转** — 修复刷新页面后点击大纲标题无法跳转、或跳一下又被拉回底部的问题（手动滚动后恢复）；站点在识别到用户真实滚动前会持续把对话拉回底部，大纲跳转现在会解除该吸底状态。
