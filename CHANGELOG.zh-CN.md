...
- **悬浮提示显示优化** — 提示更轻巧，靠近屏幕边缘时自动翻转方向，点击、按下或按 Esc 立即关闭；连续扫过相邻按钮时提示即时出现、不再重复等待；快捷按钮提示朝屏幕中心展开；大纲、会话、分组和提示词链等列表提示只在标题被截断时出现，悬停不再遮挡相邻内容。(#885)
- **更新提示方式** — 版本更新后不再自动弹出全屏更新日志。更新后右下角会出现一次性提醒条，面板 Logo 和收起状态的面板按钮会显示红点：悬停 Logo 可预览本次更新要点，点击查看完整更新日志，读完后红点消失；设置「关于」页的查看入口保留。(#887)
...
- **知乎直答支持** — 在线适配库新增知乎直答（zhida.zhihu.com），安装适配包后即可在对话页和分享页使用大纲（含用户提问）、基础导出、对话宽度、禅模式、新对话和提示词插入功能。
- **ChatGPT 长会话性能优化** — ChatGPT 站点设置新增三个渲染优化开关：代码块批量挂载（默认开启，打开或切换长会话时合并挂载代码编辑器，减少页面卡顿）、流式渲染优化（默认开启，生成回答期间暂停消息区域动画与过渡，降低渲染开销）、禁用毛玻璃效果（可选，减少长会话滚动掉帧，会轻微改变页面视觉）。(#868)

### ✨ 功能优化

- **适配包图标** — 适配包现在支持声明自定义图标图片，Notion AI 这类图标不在默认 favicon 路径的站点会显示真实 logo，不再回退为字母占位。

### 🐛 问题修复

- **适配 ChatGPT 新 DOM 结构** — 适配 ChatGPT 改版后的提问导航条与离屏空壳占位结构，修复刷新后大纲缺失完整用户提问列表，以及长会话导出静默跳过未挂载轮次、无法采集完整对话的问题。(#871, #872)
- **适配豆包新版输入框** — 适配豆包改版后的 TipTap 输入框，提示词插入、队列发送、生成中/停止检测和模型锁定在新输入区域及新版发送/停止按钮上恢复正常。(#867)
- **长会话与资产导出稳定性** — 适配 ChatGPT、豆包、DeepSeek、AI Studio 等虚拟滚动长会话导出，等待节点挂载、校验轮次连续性并定向补抓缺口，避免漏段、重复或乱序；检测到异常或超时将明确提示，ZIP 图片下载卡住时自动降级为外链，防止导出中断。(#865)
...
- **大纲锚点位置丢失** — 修复快速或重复点击「返回锚点」「去顶部」「去底部」，以及连续点击同一大纲标题时，已保存的锚点被覆盖为当前位置，导致「返回锚点」无法跳回之前位置的问题；现在锚点操作已串行化，且目标已到达时不再覆盖锚点。(#848)
- **适配 Claude 新版页面布局** — Claude 又又又一次更新了页面结构, **Fuck you, Claude!** (#823, #849)
- **适配 Grok 新 DOM 结构** — 适配 Grok 新版聊天滚动容器和主题样式选择器，修复在 Grok 上「去顶部」「去底部」「返回锚点」失效和切换亮/暗主题导致页面卡死无响应的问题。(#842, #843)
- **适配 Qwen Studio 新 DOM 结构** — 适配 Qwen Studio 新版聊天滚动容器，修复站点页面结构更新后「去顶部」「去底部」「返回锚点」失效的问题。(#)
- **适配 ChatGLM 新 DOM 结构** — 适配 ChatGLM 新版主题气泡结构，修复站点改版后亮/暗/系统主题切换失效的问题。
- **适配豆包侧边栏新 DOM 结构** — 适配豆包新版侧边栏会话列表，修复同步到会话管理的会话标题重复三遍的问题。(#846)
- **ChatGPT 提示词插入（Firefox）** — 修复在 Firefox 浏览器下向 ChatGPT 插入多行提示词时换行全部丢失的问题，插入内容现在保留原有换行结构。(#811, #838)
- **AI Studio 主题切换** — 修复在 AI Studio 上切换亮/暗主题不再生效、站点保存的主题偏好不再更新的问题，页面主题现在会重新跟随面板切换。(#844)
- **DeepSeek Harness steering 消息** — 修复在 DeepSeek Harness 生成回复过程中发送的 steering 插话消息不出现在大纲和导出内容中的问题，这类消息现在会作为用户提问正常识别。
