...
- **虚拟滚动长会话导出完整性** — ChatGPT、Doubao、DeepSeek、AI Studio 的导出现在会等待虚拟列表行挂载完成后再读取，校验轮次/行号连续性并对缺口定向补抓，长会话导出不再出现静默缺段、重复或乱序；同一 Doubao 会话重复导出的结果现在保持一致。(#780, @AstraVanX)
- **导出可能不完整提示** — 采集检测到缺失轮次/行号或历史加载触及时长上限时，导出完成后会明确提示，不再静默产出残缺文件。(#117, #780)
- **ZIP 导出资产下载超时** — 图片下载超过 30 秒未完成时降级为清单中的外链引用，单个资产挂起不再导致整个导出卡住。(#780, @AstraVanX)
