...
- **ChatGPT 导出缺轮次** — ChatGPT 改版后离屏对话轮次只保留占位空壳，缺少导出原来依赖的结构标记，导致导出长会话时静默跳过未挂载的轮次且全程不滚动加载；现在导出能识别占位空壳，逐轮滚动加载并完整采集对话内容。(#872)
- **ChatGPT 大纲用户问题抽取不全** — ChatGPT 改版了页面内的提问导航条，导致页面刷新后大纲 Tab 无法从中读取完整的用户问题列表；现已适配新版导航条，大纲恢复完整抽取用户问题。(#871)
- **页面加载时的冗余报错日志** — 修复在任何站点加载页面时都会为 Claude、Gemini 和 Gemini Enterprise 输出 "Failed to load cached patch" 报错日志的问题；这三个站点的缓存配置补丁现在也能正常应用，不再被一律拒绝。(#870)
- **豆包新版输入框适配** — 适配豆包改版后的 TipTap 输入框，提示词插入、队列发送、生成中/停止检测和模型锁定在新输入区域及新版发送/停止按钮上恢复正常。(#867)
- **虚拟滚动长会话导出完整性** — ChatGPT、Doubao、DeepSeek、AI Studio 的导出现在会等待虚拟列表行挂载完成后再读取，校验轮次/行号连续性并对缺口定向补抓，长会话导出不再出现静默缺段、重复或乱序；同一 Doubao 会话重复导出的结果现在保持一致。(#865)
- **导出可能不完整提示** — 采集检测到缺失轮次/行号或历史加载触及时长上限时，导出完成后会明确提示，不再静默产出残缺文件。(#865)
- **ZIP 导出资产下载超时** — 图片下载超过 30 秒未完成时降级为清单中的外链引用，单个资产挂起不再导致整个导出卡住。(#865)
