...
- **阶跃AI 站点支持** — 在线适配库新增阶跃AI（chat.stepfun.com），安装适配包后即可在对话页和分享页使用大纲（含用户提问）、基础导出、生成检测与停止、对话宽度、禅模式、新对话和提示词插入功能。(#839, #792, #352)

### 🐛 问题修复

- **豆包会话标题同步** — 适配豆包新版侧边栏会话列表，修复同步到会话管理的会话标题重复三遍（如“标题”变成“标题标题标题”）的问题。(#846)
- **Qwen Studio 滚动快捷功能** — 适配 Qwen Studio 新版聊天滚动容器，修复站点页面结构更新后「去顶部」「去底部」「返回锚点」失效的问题。
