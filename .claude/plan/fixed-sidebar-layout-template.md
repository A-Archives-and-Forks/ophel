...
但第一步不是默认“给输入区再补一条规则”，而是先判断：

- 输入区是否已经包含在 `main` 命中的主布局层里
- `main` 的让位是否已经让输入区一起避开了 pane

如果答案是“是”，那就**不要再单独加 `composer`**。

只有当输入区位于主内容层之外，或者主内容已经避让但输入区仍被遮挡时，才需要 `composer`。
...
1. 编辑叶子
   - 例如 `textarea`、`contenteditable`、Quill 的 `.ql-editor`
   - 这层绝对不要直接做 `padding` / `margin`
   - 否则会直接压缩可编辑区域本身

2. 输入组件壳
   - 例如包含工具栏、发送按钮、编辑器的那一整块组件
   - 如果这层本身负责宽度，直接改它容易把整块组件压窄

3. full-width wrapper
   - 指包住“居中输入组件”的外层 100% 宽容器
   - 优先对这一层做 `padding`
   - 让内部输入组件在剩余可视区里重新居中 / 重新排版

如果一个站点的输入区是“独立居中的大输入框”，优先找第 3 层，而不是第 1 层或第 2 层。
