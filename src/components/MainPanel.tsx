...
import { PANEL_EDGE_OFFSET, PANEL_RAIL_GAP, PANEL_RAIL_WIDTH, SITE_IDS, TAB_IDS } from "~constants"
...
export const MainPanel: React.FC<MainPanelProps> = ({
  ...
  const [internalActiveTab, setInternalActiveTab] = useState<string>(TAB_IDS.PROMPTS)
  const [isInitialized, setIsInitialized] = useState(false)
  const resolvedActiveTab = controlledActiveTab ?? internalActiveTab
  ...
  useEffect(() => {
    if (settings && !isInitialized && controlledActiveTab === undefined) {
      setInternalActiveTab(getFirstTab(settings.features?.order))
      setIsInitialized(true)
    }
  }, [settings, isInitialized, controlledActiveTab])
  ...
  useEffect(() => {
    ...
    if (isInitialized && tabOrder && tabOrder.length > 0) {
      if (!tabOrder.includes(internalActiveTab)) {
        setInternalActiveTab(getFirstTab(tabOrder))
      }
    }
  }, [tabOrder, isInitialized, internalActiveTab, controlledActiveTab])

  // 监听快捷键触发的 tab 切换事件
  useEffect(() => {
    const handleSwitchToOutline = () => {
      switchTab(TAB_IDS.OUTLINE)
    }
    const handleSwitchToConversations = () => {
      switchTab(TAB_IDS.CONVERSATIONS)
    }
  ...
  const renderThemeStyle = () => {
    ...
    const styleId = resolvedMode === "light" ? siteTheme?.lightStyleId : siteTheme?.darkStyleId
    ...
    return customStyle ? <style>{customStyle.css}</style> : null
  }
  ...
  return (
    <>
      ...
      {layoutVariant === "sidebar" ? (
        <div
          ref={panelRef}
          className="gh-main-panel gh-interactive sidebar-mode"
          style={{
            position: "fixed",
            top: `${PANEL_EDGE_OFFSET}px`,
            bottom: `${PANEL_EDGE_OFFSET}px`,
            ...(defaultPosition === "left"
              ? { left: `${PANEL_EDGE_OFFSET + PANEL_RAIL_WIDTH + PANEL_RAIL_GAP}px` }
              : { right: `${PANEL_EDGE_OFFSET + PANEL_RAIL_WIDTH + PANEL_RAIL_GAP}px` }),
          ...
          <div
            className="gh-panel-content sidebar-pane-content"
            style={{
              flex: 1,
              overflowY: "auto",
              padding: "0",
            ...
            {resolvedActiveTab === TAB_IDS.PROMPTS && (
              <PromptsTab
            ...
            {resolvedActiveTab === TAB_IDS.CONVERSATIONS && (
              <ConversationsTab
                manager={conversationManager}
                onInteractionStateChange={onInteractionStateChange}
              />
            )}
            {resolvedActiveTab === TAB_IDS.OUTLINE && (
            ...
            <div
              ...
              <Tooltip
                ...
                <button
                  onClick={() => {
                    // 根据当前 Tab 执行对应的刷新逻辑
                    if (resolvedActiveTab === TAB_IDS.OUTLINE) {
                      outlineManager?.refresh()
                    } else if (resolvedActiveTab === TAB_IDS.PROMPTS) {
                      // 提示词由 Zustand store 管理，自动响应数据变化，无需手动刷新
                      // 触发 UI 重新获取数据
                      promptManager?.init()
                    } else if (resolvedActiveTab === TAB_IDS.CONVERSATIONS) {
                      // 触发数据变更通知，刷新 UI
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    fontWeight: resolvedActiveTab === tab ? 600 : 400,
                    cursor: "pointer",
                    fontSize: "13px",
                    whiteSpace: "nowrap",
                    display: "flex",
