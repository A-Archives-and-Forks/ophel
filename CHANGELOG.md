...
- **会话列表渐进式加载**：文件夹下会话过多（600+ 条）时，展开文件夹不再一次性渲染全部会话。改为首次仅渲染 30 条，滚动到底部时通过 IntersectionObserver 自动加载下一批，首次展开秒开。(#369)
- **会话列表缓存优化**：使用 `useMemo` 缓存文件夹会话分组与排序结果，消除同一渲染周期内的重复 filter + sort 计算。
- **Tag 查找 O(1) 化**：会话项的标签解析从 `tags.find()` 线性查找改为 `Map.get()` 常量时间查找。
- **会话项 React.memo**：将会话项抽取为独立 `ConversationItem` 组件并包裹 `React.memo`，减少文件夹切换、搜索等操作时的不必要子树重渲染。
