...
- **页面宽度上限提升至 99%**：将页面宽度和用户提问宽度的最大可调百分比上限从 94% 提升至 99%，满足希望内容接近全宽显示的用户需求。（#447）
- **页面宽度响应式适配（窄屏自动扩展）**：当页面宽度单位为百分比时，若视口宽度不大于 480px（手机等窄屏设备），内容宽度会自动切换为 95%，避免按百分比设置后内容在手机屏幕上过于狭窄的问题。（#447）

- **面板展开时自动隐藏快捷按钮组**：在设置页「快捷按钮」标签下新增开关，开启后展开主面板时快捷按钮组会自动收起，关闭面板后恢复显示，让界面更简洁。
- **快捷按钮设置分组更清晰**：将「隐藏快捷按钮组」开关和「不透明度」调节独立拆分为一个设置卡片，置于按钮排序列表上方，更容易发现和使用。
...
- **DeepSeek 页面宽度**：调整 AI 回复与输入框宽度，使两者保持一致。
