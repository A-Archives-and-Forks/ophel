...
- **界面整体样式优化**：优化诸多 UI 细节。
- **设置页社区入口升级**：左下角由单一 GitHub 链接升级为四个社区快速入口（商店评分、GitHub、Ko-fi、Discord），配有品牌色轮流心跳动画，鼠标悬浮时高亮填充，同步整合主题与语言切换为极简 Dock 布局。
- **关于页面视觉打磨**：操作按钮改为幽灵风格（悬浮时实色），顶部口号升级为渐变胶囊徽章，侧边栏导航激活项新增指示条，主题卡片选中态更立体。
- **功能提示导览强化**：在面板、大纲标签页等位置新增高级功能提示气泡（MagicCodex），引导用户发现隐藏功能，同步扩充功能提示常量库与 i18n。(#416)
- **面板 Tab 图标尺寸优化**：将大纲、会话、提示词标签栏图标从 14px 调大至 16px，提升视觉清晰度。

### 🐛 问题修复

- **更新提醒重复弹出**：修复扩展在后台更新（Chrome 商店推送）后，用户关闭"插件已更新"弹窗，几分钟后弹窗仍反复出现的问题。根本原因是旧版 Content Script 在上下文失效后持续抛出异常，触发弹窗重新显示。现点击关闭后，页面内不再重复弹出，刷新页面后完全恢复正常。
- **设置滑块恢复默认后刷新回弹**：修复在"基本设置-面板"及站点设置中，点击滑块"恢复默认"按钮即时生效、但刷新页面后又恢复为旧值的问题。根本原因是恢复默认时先触发预览（`onPreviewChange`），预览阶段临时屏蔽持久化写入，导致同一调用栈内的正式提交也被跳过。现恢复默认直接写入目标值，不再经过预览通道。
