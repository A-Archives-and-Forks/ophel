...
- **面板模式切换快捷键**：新增 `Alt+M`（Mac：`⌥M`）快捷键，可在「自动吸附」与「悬浮」模式之间一键切换。切换后显示 Toast 提示当前模式。快捷键出现在设置页面「快捷键 → 面板」分类，可自定义绑定。(#{{PR}})

### ✨ 功能优化

- **锚点按钮点击弹性动效**：主面板底栏锚点按钮与快捷浮动锚点按钮在点击时新增弹性缩放动画，提供更明确的即时视觉反馈。连续快速点击时每次都能重播动效。动效尊重系统「减少动态效果」偏好设置。(#426)
