...
- **会话列表性能优化**：文件夹下大量会话展开不再卡顿，采用渐进式加载，首次打开秒开。(#369)
- **全局搜索性能优化**：减少搜索结果的重复渲染与高亮计算开销，提升搜索流畅度。
- **主题适配精细化**：原生主题不再覆盖整个页面背景，仅对侧边栏染色，避免影响 AI 平台主内容区的原始外观。
- **内置 Inter 字体**：扩展版内置 Inter Variable 字体，提升 UI 排版品质；油猴版自动回退到系统字体，无额外开销。

### 🐛 问题修复

- **修复韩文显示为方块**：补充面板字体栈中缺失的韩文/CJK 字体回退，解决油猴脚本 Shadow DOM 内韩文字符无法正常渲染的问题。(#373)
- **修复 TypeScript 类型错误**：将 `NodeJS.Timeout` 替换为 `ReturnType<typeof setTimeout>`，消除浏览器环境下的 18 处命名空间报错。
- **快捷按钮液态收缩位置漂移**：修复快捷按钮组在收缩为“小水滴”后，通过设置面板禁用某个快捷按钮会导致位置自动下漂的问题；现仅在位置数值真正变化时才同步持久化坐标，并在尺寸变化后及时回写校正后的新位置，避免因同值对象重建和高度变化叠加造成错位。
- **快捷按钮液态图标固定为 Ophel Logo**：修复快捷按钮组拖动排序后，液态收缩态的小水滴会错误显示为顶部第一个按钮图标的问题；现在收缩态始终固定展示 Ophel Logo，不再受按钮顺序影响。
