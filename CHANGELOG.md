...
- **WebDAV 备份设置 UI 全面优化**：WebDAV 设置页布局重新设计，新增常见服务商快速预填（坚果云、InfiniCloud、Nextcloud 等），配置操作按钮整合为左（测试/保存）右（恢复/立即备份）对齐布局；备份列表操作按钮改为描边幽灵风格（蓝色恢复、红色删除）；恢复提示框改为中性灰色样式；图标替换为风格统一的填充型 SVG（刷新、恢复、备份）。
...
- **豆包同步删除失效**：修复豆包升级新版页面结构后，同步删除会话功能失效的问题。主要修复三处：① 菜单项检测改用 `text-dbx-function-danger` 样式类代替已废弃的 `data-testid` 属性；② 确认弹窗检测改用 `data-slot="dialog-content"` 新属性代替文本匹配；③ 确认按钮识别改用危险色样式类，同时补全繁体中文（刪除）和英文（delete）文本兜底，修复繁体界面下无法触发确认的问题。
- **快捷按钮组首次出现闪动**：修复刷新页面时快捷按钮从顶端滑落到正确位置的动画问题。
